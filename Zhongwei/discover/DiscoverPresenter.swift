...
class DiscoverPresenter:Presenter{
    ...
    func getPrizeRecordUrl() -> Observable<String>{
        var unionid = self.app.globalData!.unionid
        var headImgUrl = self.app.globalData!.headImgUrl
        var nickName = self.app.globalData!.nickName
        ...
        return getSid()
            .map{ sid in
                ...
                return "\(self.baseUrl)resources/app/scan-qr-code/redeem-record.html?\(timeStamp)=&sid=\(sid)"
        }
    }
    
    
    func getPrizeData(code:String)  -> Observable<AlertDataResult>{
        var unionid = self.app.globalData!.unionid
        var headImgUrl = self.app.globalData!.headImgUrl
        var nickName = self.app.globalData!.nickName
        //return getSid(unionid: unionid, headimgurl: headImgUrl, nickname: nickName)
        return getSid()
            .flatMap{
                ...
                return Observable<AlertDataResult>.create {
                    ...
                    var result = "a"
                    Alamofire.request("\(self.baseUrl)mobile/Redeem/doRedeem?sid=\(sid)&code=\(code)").responseString{
                        response in
                                ...
                                if (prizeEntity.code != 200) {
                                    ...
                                    if let prizeData = PrizeData.deserialize(from: dataString){
                                        if (prizeData.ret == "1307" || prizeData.ret == "1305") {
                                            ...
                                            var time = self.getTimeStamp(timeString:prizeData.content?.transacId as! String)
    ...
    func getTimeStamp(timeString:String) -> String{
